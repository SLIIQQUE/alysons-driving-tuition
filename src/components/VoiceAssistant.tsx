...
export default function VoiceAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm here to help you with driving lessons. Ask me anything or type a message!",
    },
  ]);
  ...
  const cleanup = useCallback(() => {
    ...
    if (processorRef.current && recordContextRef.current) {
      try {
        processorRef.current.disconnect();
        recordContextRef.current.destination.disconnect();
      } catch (e) {
      ...
      const startAudioCapture = async () => {
        try {
          ...
          processor.onaudioprocess = (e) => {
            if (!isSessionActiveRef.current || !isAudioProcessingRef.current) return;
            ...
            try {
              ...
              // Session likely closed, stop audio capture immediately
              const errorMessage = err instanceof Error ? err.message : String(err);
              if (!errorMessage.includes('CLOSING') && !errorMessage.includes('CLOSED')) {
                console.log("Audio send error:", errorMessage);
              }
              isAudioProcessingRef.current = false;
              isSessionActiveRef.current = false;
            }
          ...
          onmessage: async (response: any) => {
            try {
              ...
              if (content?.inputTranscription?.text) {
                setMessages((prev) => [
                  ...prev,
                  { role: "user", content: content.inputTranscription.text },
                ]);
              }
              if (content?.outputTranscription?.text) {
                setMessages((prev) => [
                  ...
                  {
                    role: "assistant",
                    content: content.outputTranscription.text,
                  },
                ]);
              }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/35 text-[11px]">
                  ...
                  {voiceState === "idle" &&
                    (mode === "voice" ? "Tap mic to talk" : "Type a message")}
                </p>
              </div>
            </div>
        ...
        <div className="h-[320px] overflow-y-auto p-4 space-y-3">
          <AnimatePresence mode="popLayout">
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={`max-w-[88%] px-4 py-3 rounded-[20px] text-[13px] leading-relaxed ${
                    message.role === "user"
          ...
          {mode === "voice" ? (
            <div className="flex flex-col items-center gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.92 }}
                onClick={
                  voiceState === "idle" ? startVoiceSession : stopVoiceSession
                }
